...
class SentenceBertRecommendationEngine(AbstractNLPRecommendationEngine):
    ...
    def calculate_compatibility(self, user_skills: list, required_skills: list) -> dict:
        ...
        for r_skill in required_skills:
            # Check if any user skill genuinely matches this required skill
            is_matched = any(
        ...
        if total == 0:
            ...
            match_score = raw  # Exact dynamic percentage based strictly on matching skills
        ...
        return {
            ...
            "matched_skills": matched,       # Only skills student actually has
    ...
    def generate_recommendations(self, user_skills: list) -> list:
        ...
        try:
            ...
            for o in qs:
                # Always use the real required_skills from the DB — never guess
                req_skills = []
                if isinstance(o.required_skills, list):
                    req_skills = [s for s in o.required_skills if isinstance(s, str) and s.strip()]
                elif isinstance(o.required_skills, str):
                    try:
                        import json
                        parsed = json.loads(o.required_skills)
                        req_skills = parsed if isinstance(parsed, list) else []
                    except Exception:
                        pass

                domain = "Software Dev" if o.opportunity_type == "INTERNSHIP" else "Social & Community"
                opps.append({
                    "id": str(o.id),
                    "title": o.title,
        ...
        if not opps:
            ...
            for r in opp_rows:
                opp = dict(r)
                # Read required_skills from DB — it may be stored as JSON string
                raw_skills = opp.get("required_skills") or opp.get("skills_required") or ""
                if isinstance(raw_skills, list):
                    ...
                    try:
                        ...
                        req_skills = parsed if isinstance(parsed, list) else [s.strip() for s in raw_skills.split(",") if s.strip()]
                    except Exception:
                        req_skills = [s.strip() for s in raw_skills.split(",") if s.strip()]
                else:
                    # No required skills defined — infer from type
                    if opp.get("opportunity_type", "").lower() in ("ngo", "volunteer", "community"):
                        req_skills = ["Communication", "Social Work", "Teamwork"]
                    else:
                        req_skills = ["Python", "SQL", "Git"]

                domain = "Software Dev" if opp.get("opportunity_type") == "Internship" else "Social & Community"
                opps.append({
                    ...
                    "stipend": "₹35,000 / month" if opp.get("opportunity_type") == "Internship" else "Unpaid / Volunteer",
        ...
        for opp in opps:
            ...
            # Only include if there is at least some match OR the student has no skills
            results.append({
                "id": opp["id"],
                "title": opp["title"],
...
class PlacementReadinessService(AbstractPlacementReadinessService):
    def calculate_readiness_score(self, user_skills: list, student_email: str = None) -> dict:
        ...
        # -------------------------------------------------------------
        if app_count == 0:
            app_score = 0  # No baseline; new students start at 0
        elif app_count == 1:
            app_score = 45
        elif app_count == 2:
            app_score = 70

        ...
        try:
            ...
            for row in all_apps:
                ...
                if target_email:
                    ...
                    org_str = (a_dict.get('organization') or 'Unknown Organization').strip()
                    title_str = (a_dict.get('opportunity_title') or 'General Opportunity').strip()
                    company_key = f"{org_str.lower()}___{title_str.lower()}"
                    ...
                    app_obj = {
                        ...
                        'opportunity_title': title_str,
                        'status': app_status or 'Applied',
                        'applied_date': str(a_dict.get('applied_date') or '')[:10],
                        'is_shortlisted': is_shortlisted
                    }

                    seen_map = application_stats[target_email]['_seen_companies']
                    if company_key not in seen_map:
                        seen_map[company_key] = app_obj
                        application_stats[target_email]['applications'].append(app_obj)
                        application_stats[target_email]['companies_applied'] += 1
                        if is_shortlisted:
                            application_stats[target_email]['shortlisted'] += 1
                    else:
                        # If duplicate has a higher status (e.g. shortlisted/interview), upgrade it
                        if is_shortlisted and not seen_map[company_key]['is_shortlisted']:
                            seen_map[company_key]['is_shortlisted'] = True
                            seen_map[company_key]['status'] = app_status
                            application_stats[target_email]['shortlisted'] += 1
    ...
    def review(self, request, pk=None):
        ...
        if req:
            ...
            try:
                ...
                # Insert dynamic notification specifically for this student
                import random
                verif_notif_id = f"notif-verif-{int(time.time())}-{random.randint(100, 999)}"
                cursor.execute("""
                    INSERT INTO notifications (id, student_email, title, message, timestamp, `read`, type, created_at)
                    VALUES (?, ?, ?, ?, ?, 0, 'verification', NOW())
                """, (
                    verif_notif_id,
                    req.email,
                    f"Profile Verification: {new_status.title()}",
                    f"Faculty moderator updated your profile verification status to '{new_status.title()}'." + (f" Note: {reason}" if reason else ""),
                    "Just now"
                ))
